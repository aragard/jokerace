...
Our initial job was to collect the dataset ([available here](https://docs.google.com/spreadsheets/d/17Evor8Xl5_Ks9OpifEQxARS6MfsvU1sUTM54tS1JK8c/edit#gid=705342398)). We found the contract for each JokeRace, queried each of the transactions and then pulled out the data. Dune Dashboards were utilized to extract all of the transactions for the 4 contracts in question:
...
## Libraries for Getting Started

### with Topic Analysis

In this tutorial, we will use the following R packages to perform topic analysis:
...
We use the following R packages for our high-level text-analysis:
...
If you need additional guidance getting set up, please refer to our earlier tutorials on using R Studio and R.
...
You can view the file in a spreadsheet application like Excel or use `head(propfinal)` to preview the data. The contents of the CSV are as shown in the image below:
...
There were 1463 submissions and the DTM had a vocab of 763. (If you're unfamiliar with DTMs refer to the "Compute DTM" section).
...
We are effectively performing topic analysis six times (once for each dataframe RF4, GI5, etc.). We will use a main for-loop and save the `results` in a variable located outside of the body of this for-loop (see next slide to see how the loop is set up).
...
Here we are creating a one-column matrix containing 50 rows, converting it to a one-column dataframe and saving it in a variable called `results`. In the second line, we name the column heading, `TermIdx`.
...
The first lines of code that we will add to the body of the loop are for extracting the text data from the "Content" column of `propfinal`. Recall that `propfinal` contains all the data while `contestdf` allows us to keep track of the respective JokeRace contracts and the number of topics for each topic analysis. In the first line, we take all of the text data concerning a given contract and store in an appropriately named variable named `textdata`. In the second line, we encode this data as "bytes".
...
## Interpreting Topic Analysis Results

Now we have a much smaller dataset that summerizes the original dataset (50 rows vs 20k rows). Our task is to determine whether it is representative of our objectives. Usually the output will have to be reviewed multiple times to further remove noise and tune the results by interpreting them based on knowledge of the domain. In the beginning of the interpretation process, you will not know ahead of time what patterns and specific results are most useful but ideally you will have prior knowledge of the domain to act as a guide. A simple way to get started with this qualitative research is to print out the dataset and annotate it the way you would annotate a book or another document. We suggest performing the exercise below for practice.

### Optional Exercise

Highlight words that standout, make notes about the features (ex. what topics commonly occur together? What topics can we group together? Are there any notable patterns across the topics? etc.). Play with the interpretation a bit and see if you can come up with your own version of a proposal containing suggestions for 5 short-term goals, 5 long-term goals, an overarching mission statement and a vision statement for the DAO.

Note: For additional examples of text analysis interpretation, see the interpretations for the n-grams, verbs and nouns in Part 2. It might help to interpret the higher-level text analysis first to get a better idea of what to look out for during the topic analysis.
...
## Final Body of For-loop(cont'd 3)
```{r eval=FALSE}
  # Tokenise text data and preprocess
  tokens <- unnest_tokens(textdata, word, text) %>%
      mutate(word = str_replace_all(word, "[^[:alnum:]]", "")) %>%
...
## Final Body of For-loop(cont'd 4)
```{r eval=FALSE}
